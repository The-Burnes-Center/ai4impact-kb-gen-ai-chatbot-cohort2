...
export default class ClaudeModel{
  ...
  async getNoContextStreamedResponse(prompt) {
    ...
    try {
      ...
      console.error("Caught model invoke error: ", e)
    }
  ...
  async getNoContextResponse(prompt, len){
    ...
    try {
      const command = new InvokeModelCommand({ body: JSON.stringify(payload), contentType: 'application/json', modelId: this.modelId });
      const apiResponse = await this.client.send(command);
  ...
  // async *generateStreamedResponse(system, history) {
